  ...
  <FirebaseApp {firebase} on:initializeApp={init}>
    <div class="h-100 mw5 w-25 bg-near-black near-white pa4">
      ...
      <h3 class="f6 tracked fw9 ttu">
        {#if isDetail}
          <span on:click={resetForm}>-</span>
          Fields
          <span on:click={addField}>+</span>
        {:else}My Forms{/if}
      </h3>
        ...
        {#if isDetail}
          {#each fields as field}
            <li class="lh-copy pv2 flex">
              ...
              <span contenteditable="true" class="pa1">{field.value}</span>
            </li>
          {/each}
        {:else if forms}
          {#each forms as form}
            <li class="lh-copy pv2" on:click={() => showPdf(form.id)}>
              {form.id}
            </li>
          {/each}
        {/if}
